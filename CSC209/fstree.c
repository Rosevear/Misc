...
//Global counter variable to periodically call markand_sweep in do_fstree.c
int count_tree = 0;
...
void add_node(char *path, char *name) {
    ...
    //Since the link has been added to the list of links, it is now reachable by mark_and_sweep.
    //So we can unblock the signal.
  //  printf("You can mark and sweep!\n");
    if (sigprocmask(SIG_UNBLOCK, &signal_to_block, NULL) == -1) {
        	perror("sigprocmask");
            exit(1);
         }
//To test that the signal is unblocked
//        usleep(1000000);
//        usleep(1000000);
//       usleep(1000000);
//        usleep(1000000);
